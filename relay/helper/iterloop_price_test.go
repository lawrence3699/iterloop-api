...
func TestHandleGroupRatioUsesFaroBaselineForCombinedIterLoopGroup(t *testing.T) {
	gin.SetMode(gin.TestMode)
	ctx, _ := gin.CreateTestContext(httptest.NewRecorder())

	codex := &relaycommon.RelayInfo{UsingGroup: "combined-standard", OriginModelName: "gpt-5.5"}
	assert.Equal(t, 1.0, HandleGroupRatio(ctx, codex).GroupRatio)

	claude := &relaycommon.RelayInfo{UsingGroup: "combined-standard", OriginModelName: "claude-sonnet-4-6"}
	assert.Equal(t, 1.0, HandleGroupRatio(ctx, claude).GroupRatio)
